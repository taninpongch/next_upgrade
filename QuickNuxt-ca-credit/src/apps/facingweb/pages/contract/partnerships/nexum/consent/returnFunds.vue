    ...
    <MuiPage>
      ...
      <BizShareManaForm :schema="validationRules" :state="dataForm" @submit="onSubmit">
        <MuiCard>
          ...
          <BizSharePartyHero :src="displayImage" :title="data?.baInfo?.name" :subtitle="consentDate" />
        </MuiCard>
        <MuiCard>
...
getData('get-ca-consent-returnfunds').then(async () => {
...
const linkRef = [{
  label: 'ให้กับ',
  value: data.value?.returnfundsInfo?.name ? data.value.returnfundsInfo.name : "-",
}, {
  label: 'วงเงินทั้งหมด',
  value: data.value?.returnfundsInfo?.totalAmount ? manaLib.toText(data.value.returnfundsInfo.totalAmount) : "0 THB",
}, {
  label: 'ขอเพิ่มงบ',
  value: data.value?.returnfundsInfo?.returnAmount ? manaLib.toText(data.value.returnfundsInfo.returnAmount) : "0 THB",
  danger: true,
}]
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  if (event.data) {
    ...
    await postSubmitConsentData(formdata, "post-ca-consent-returnfunds", data.value.serviceSubscriptionId);
  }
}
</script>