...
export const useTopupStore = defineStore('topupStore', () => {
  const subscriptionId = "topup";

  var getUrl = ref("");
  const { data: dataTopup, error: errorMsg, pending, execute: executeGet } = useMFetch<any>(getUrl, { method: "GET", headers: { "Accept": "application/json" } });
