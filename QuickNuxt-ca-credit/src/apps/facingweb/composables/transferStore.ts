...
export const useTransferStore = defineStore('transferStore', () => {
  const subscriptionId = "transfer";

  var getUrl = ref("");
  const { data: dataTransfer, error: errorMsg, pending, execute: executeGet } = useMFetch<any>(getUrl, { method: "GET", headers: { "Accept": "application/json" } });
