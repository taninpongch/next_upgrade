...
export const useLogStore = defineStore('logStore', () => {
  ...
  // TODO move to walletStore???
  const subscriptionId = "wallets";

  var getUrl = ref("");
  const { data: dataLog, error: errorMsg, pending, execute: executeGet } = useMFetch<any>(getUrl, { method: "GET", headers: { "Accept": "application/json" } });
  ...
  return {
    getData, postData, dataLog, pending
  }
})