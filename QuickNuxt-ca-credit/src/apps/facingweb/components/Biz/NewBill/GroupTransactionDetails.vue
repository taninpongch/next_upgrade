<template>
    <BizShareSessionTitle label="รายการทั้งหมดที่ต้องจ่าย"
        :button="{ icon: 'i-lucide-calendar-fold', label: 'ดูเพิ่ม' }" class="py-2" @on-tapped="navigate" />
    <MuiCard>
...
const navigate = async () => {
    if ((props.data?.type == 'delivery' || props.data?.type == 'deliveryRefund' || props.data?.type == 'deliveryCheckout') && props.data) {
        ...
        await goVisit("bill", "bill-employeelist", props.data?.session.sessionId);
    }
...
const txsummary = computed(() => {
    ...
    else if (props.data?.isReceiver == false && props.data?.type == 'CreditBillPayment') {
        return [{
            description: 'รวมค่าสินค้า',
            value: lib.toText(props.data?.session?.amount),
        }, {
