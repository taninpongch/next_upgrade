...
getData('get-ba-kym-basic-select-type-dlg').then(async () => {
  ...
  getOptionDialogSelection().then(() => {
    dataLocal.value?.forEach((newItem: any) => {
      const item: any = dataForm.type.find((currentItem: any) => currentItem.label === newItem);
      if (item) {
        item.isCheck = true;
      }
    })
...
// type: z.array(z.object({ isCheck: z.boolean(), label: z.string(), value: z.string(), })

const validationRules = z.object({
  type: z.array(z.object<any>({ isCheck: z.boolean(), label: z.string(), value: z.string(), }))
});
