<template>
    ...
    <div v-else>
        <MuiPage>
            <MuiHeader title="คำขออนุมัติยกเลิกพักงาน" />
                ...
                <MuiCard>
                    <BizShareValidateItemList v-model="dataForm.acceptances" name="acceptances"
                        ...
                        <BizShareInputMultiCheckbox v-model="checkboxModel" heading="ผู้ถูกเพิ่มเป็นพนักงาน"
                            :data="data?.employeeList?.approval" flow="unsuspend" />
...
getData("get-consent-unsuspend-employee-list").then(async () => {

});
// store selected values as array of ids (strings)
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    if (event.data) {
        ...
        await postSubmitConsentData(formdata, "post-consent-unsuspend-employee-list", data.value.serviceSubscriptionId);
    }
}
</script>
