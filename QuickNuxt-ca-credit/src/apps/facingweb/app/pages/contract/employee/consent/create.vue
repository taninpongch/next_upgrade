<template>
  ...
  <div v-else>
    <MuiPage>
      <MuiHeader title="คำขออนุมัติเพิ่มพนักงาน" />
      <BizShareManaForm :schema="validationRules" :state="dataForm" @submit="onSubmit">
        <MuiCard>
          <BizSharePartyHero :src="baImage" :title="data?.baInfo?.name" :subtitle="consentDate" />
        </MuiCard>
        <MuiCard>
          <BizShareValidateItemList v-model="dataForm.acceptances" name="acceptances" :show-errors="submitAttempted">
            <BizShareInputMultiCheckbox v-model="checkboxModel" heading="ผู้ถูกเพิ่มเป็นพนักงาน"
              :data="data?.employeeList?.approval" flow="create" />
...
getData("get-consent-create-employee-list").then(async () => {
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  if (event.data) {
    ...
    await postSubmitConsentData(formdata, "post-consent-create-employee-list", data.value.serviceSubscriptionId);
  }
}
