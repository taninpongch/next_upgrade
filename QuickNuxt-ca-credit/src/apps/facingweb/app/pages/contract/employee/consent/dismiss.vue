<template>
    ...
    <div v-else>
        <MuiPage>
            <MuiHeader title="คำขออนุมัติปลดพนักงาน" />
            <BizShareManaForm :schema="validationRules" :state="dataForm" @submit="onSubmit">
                ...
                <MuiCard>
                    <BizShareValidateItemList v-model="dataForm.acceptances" name="acceptances"
                        ...
                        <BizShareInputMultiCheckbox v-model="checkboxModel" heading="ผู้ถูกปลดจากการเป็นพนักงาน"
                            :data="data?.employeeList?.approval" flow="dismiss" />
...
getData("get-consent-dismiss-employee-list").then(async () => {
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    ...
    console.log("onSubmit", dataForm.value.acceptances);
    ...
    if (event.data) {
        ...
        await postSubmitConsentData(formdata, "post-consent-dismiss-employee-list", data.value.serviceSubscriptionId);
    }
}
</script>
